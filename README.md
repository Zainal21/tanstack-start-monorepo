...
an Elysia API, TanStack Start frontend, and Postgres — with a small todo
vertical slice that demonstrates the architecture without hiding it behind
abstractions.
...
| API      | [Elysia](https://elysiajs.com) on Bun + [Eden](https://elysiajs.com/eden/overview) for end-to-end types                                     |
...
These values come from `apps/server/.env`; no credentials are hardcoded in the
application. Set `SEED_DEFAULT_USER=true` together with `SEED_USER_NAME`,
...
2. Replace the app title, favicon, and the example todo domain with your product.
...
| Add an OIDC provider      | Add `socialProviders` in `apps/server/src/lib/auth.ts`; re-run `auth:generate` + a migration if the schema changed |
...
| Run the full quality gate | `bun run validate` (format + lint + types + tests + build)                                                         |
| Audit high-risk packages  | `bun run security:audit`                                                                                           |

## Quality baseline

`bun run validate` provides a platform-independent gate for formatting,
linting, strict type checks, unit tests, and production builds.
